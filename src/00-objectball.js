...
function numPointsScored(playerName){
    ...
    for (const gameKey in game){
        const teamObj = game[gameKey] //team obj to home or away
        const playerObj = teamObj.players //playerObj to players on home or away
        for (const playerKey in playerObj){  
            if (playerKey === playerName){ //searches player from home or away & name
                return playerObj[playerName].points //stats
            }
        }
    }
}
